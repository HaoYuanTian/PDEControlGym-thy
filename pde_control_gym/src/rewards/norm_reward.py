...
class NormReward(BaseReward):
    ...
    This reward offers :math:`L_1, L_2, L_\infinity` norms that can be implemented in a variety of different ways according to the parameters. 
    ...
    def reward(self, uVec: np.ndarray =None, time_index: int = None, terminate: Optional[bool] =None, truncate: Optional[bool] =None, action: Optional[float] =None):
        ...
        if uVec == None:
            raise Exception("Class NormReward attempted to call reward function and received a None vector to compute on")
        if time_index == None:
            raise Exception("Class NormReward attempted to call reward function and received a None time_index parameter to identify the reward step")
        ...
        match self.horizon:
            case "temporal":
                return -np.linalg.norm(uVec[time_index], ord=self.norm) / self.norm_coeff
            case "differential":
                if time_index > 0:
            ...
            case "t-horizon":
                ...
                if time_index > self.t_horizon_length:
                    for i in range(0, self.t_horizon_length):
                        # 计算最后 self.t_horizon_length 步，一起相加再处以平均数
